...
void traffic_on_one_road() {
    ...
    vector<int> route;
    cars_on_the_road.insert(cars_on_the_road.end(), car(0, 100000000, route, 0, 0, 0));
    for(int i = 1; i < N; i++){
        cars_on_the_road.insert(cars_on_the_road.end(), car(i, -7.5 * i, route, 0, 0, 0));
    }
...
//            printf("%lf %lf %lf %lf\n", vel_current, vel_forward, position_current, position_forward);
            ...
            it->set_acceleration(changed_acceleration);
            ...
            it->set_velocity(changed_velocity);
            it->set_position_on_the_current_road(changed_position);
